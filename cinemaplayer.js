...
function cinemaPlayerInit(elem) {
  ...
  // начало
  function loadVastAd(vastUrl, playerElement) {
    fetch(vastUrl)
      .then(response => response.text())
      .then(vastXML => {
        const parser = new DOMParser();
        const mediaFileUrl = parser.parseFromString(vastXML, "text/xml").querySelector("MediaFile").textContent;

        if (mediaFileUrl) {
          const video = document.createElement('video');
          video.src = mediaFileUrl;
          // video.controls = true;
          video.muted = true;
          video.autoplay = true;
          ...
          muteButton.style.right = '40px';
          muteButton.style.background = 'rgba(255, 255, 255, 0.5)';
          ...
          timerDisplay.style.borderRadius = '50%';
          timerDisplay.style.width = '25px';
          timerDisplay.style.height = '25px';
          timerDisplay.style.display = 'flex';
          timerDisplay.style.alignItems = 'center';
          timerDisplay.style.justifyContent = 'center';
          playerElement.appendChild(timerDisplay);
          ...
          console.error("No MediaFile found in VAST response");
          loadMainContent(playerElement);
        }
      })
      .catch(error => {
  ...
  // console.log(cinemaplayer, 'cinemaplayer');
  function loadMainContent(playerElement) {
    // console.log(playerElement, 'playerElement loadMainContent');
    // console.log(c.innerHTML, 'c.innerHTML loadMainContent');
    // console.log(c.parentNode, 'c.parentNode loadMainContent');
    if (!playerElement) {
      // console.log(cinemaplayer.innerHTML, 'cinemaplayer.innerHTML loadMainContent');
